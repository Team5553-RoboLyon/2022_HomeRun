...
#define NIsArrayAwkward(parr, pb, b) (NCheckArrayIntegrity(parr, pb, b) < 0 ? 1 : 0)
	...
	inline NBYTE *NGetArrayIndexPtr(const NARRAY *parray, const Nu32 index)
	{
		NErrorIf(index >= parray->Size, NERROR_ARRAY_INDEX_BEYOND_LIMITS);
		return parray->pFirst + index * parray->ElementSize;
	}
	// inline	Nu32	NGetArrayPtrIndex(const NARRAY *parray,NBYTE*ptr){Nu32 i; NBYTE*ptrb; ptrb = parray->pFirst; for(i=0;i<parray->Size;i++){if(ptrb==ptr){return i;}ptrb+=parray->ElementSize;} 	assert(i<parray->Size); return NVOID;}
	inline Nu32 NGetArrayPtrIndex(const NARRAY *parray, const NBYTE *ptr)
	{
