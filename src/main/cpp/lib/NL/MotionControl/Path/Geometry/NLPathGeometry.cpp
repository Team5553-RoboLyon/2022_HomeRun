...
NLPATH_GEOMETRY::NLPATH_GEOMETRY()
{
	m_flags = 0;
	m_ds = 0.0f;
	NSetupArray(&m_keyPointsArray, DEFAULT_NLPATH_KEYPOINTS_ARRAYCAPACITY, sizeof(NLPATH_POINT));
	NSetupArray(&m_primitivesArray, DEFAULT_NLPATH_PRIMITIVES_ARRAYCAPACITY, sizeof(NLPATH_PRIMITIVE));

#ifdef _NEDITOR
	m_KeyPointExtend.x = DEFAULT_NLPATH_GEOMETRY_KEYPOINT_XEXTEND;
	m_KeyPointExtend.y = DEFAULT_NLPATH_GEOMETRY_KEYPOINT_YEXTEND;
	...
	NSetColorf(&m_KeyPointColor, DEFAULT_NLPATH_GEOMETRY_KEYPOINT_COLOR);
	NSetColorf(&m_KeyPointIDColor, DEFAULT_NLPATH_GEOMETRY_KEYPOINTID_COLOR);
	NSetColorf(&m_WarningColor, DEFAULT_NLPATH_GEOMETRY_WARNING_COLOR);
#endif
}

NLPATH_GEOMETRY::NLPATH_GEOMETRY(const Nu32 keypoints_capacity, const Nu32 primitives_capacity)
{
	m_flags = 0;
	m_ds = 0.0f;
	NSetupArray(&m_keyPointsArray, keypoints_capacity, sizeof(NLPATH_POINT));
...
NLPATH_GEOMETRY::~NLPATH_GEOMETRY()
{
	NClearArray(&m_keyPointsArray, NULL);
	NClearArray(&m_primitivesArray, NULL);
}

NLPATH_GEOMETRY::NLPATH_GEOMETRY(NLPATH_GEOMETRY const &tocopy)
{
	m_flags = tocopy.m_flags;
	m_ds = tocopy.m_ds;
	NSetupArray(&m_keyPointsArray, tocopy.m_keyPointsArray.Capacity, sizeof(NLPATH_POINT));
	NSetupArray(&m_primitivesArray, tocopy.m_primitivesArray.Capacity, sizeof(NLPATH_PRIMITIVE));

	NCopyArray(&m_keyPointsArray, &tocopy.m_keyPointsArray);
	NCopyArray(&m_primitivesArray, &tocopy.m_primitivesArray);

#ifdef _NEDITOR
	m_KeyPointExtend = tocopy.m_KeyPointExtend;
	m_ClothoidDrawAccuracy = tocopy.m_ClothoidDrawAccuracy;
	m_ArcDrawAccuracy = tocopy.m_ArcDrawAccuracy;

	m_SegmentColor = tocopy.m_SegmentColor;
	m_ClothoidColorA = tocopy.m_ClothoidColorA;
	m_ClothoidColorB = tocopy.m_ClothoidColorB;
	m_ArcColor = tocopy.m_ArcColor;
	m_KeyPointColor = tocopy.m_KeyPointColor;
	m_KeyPointIDColor = tocopy.m_KeyPointIDColor;
	m_WarningColor = tocopy.m_WarningColor;
#endif
}
...
NLPATH_GEOMETRY &NLPATH_GEOMETRY::operator=(const NLPATH_GEOMETRY &from)
{
	...
	m_ds = from.m_ds;
	NCopyArray(&m_keyPointsArray, &from.m_keyPointsArray);
...
Nu32 NLPATH_GEOMETRY::write(FILE *pfile)
{
	...
	header.m_ds = m_ds;
	// header.m_keyPointsArraySize		= m_keyPointsArray.Size;
	// header.m_primitivesArraySize	= m_primitivesArray.Size;;
	NGetArrayBounds(&header.m_keyPointsArrayBounds, &m_keyPointsArray);
	NGetArrayBounds(&header.m_primitivesArrayBounds, &m_primitivesArray);
	if (fwrite(&header, sizeof(NLPATH_GEOMETRY_HEADER), 1, pfile) != 1)
		return 0;

	// 3) �criture m_keyPointsArray � la main et en une fois
	if (m_keyPointsArray.Size)
	{
		if (fwrite(m_keyPointsArray.pFirst, m_keyPointsArray.ElementSize, m_keyPointsArray.Size, pfile) != m_keyPointsArray.Size)
			return 0;
	}
...
Nu32 NLPATH_GEOMETRY::read(FILE *pfile)
{
	...
	case NGETVERSION_MAIN(VERSION_NLPATH_GEOMETRY_HEADER):
		...
		if (NIsArrayCorruptedOrInconsistent(&m_keyPointsArray, &header.m_keyPointsArrayBounds, NTRUE))
			return 0;
		if (NIsArrayCorruptedOrInconsistent(&m_primitivesArray, &header.m_primitivesArrayBounds, NTRUE))
			return 0;

		NResizeArray(&m_keyPointsArray, header.m_keyPointsArrayBounds.Size, NULL, NULL);
		NResizeArray(&m_primitivesArray, header.m_primitivesArrayBounds.Size, NULL, NULL);

		if (m_keyPointsArray.Size)
		{
			if (fread(m_keyPointsArray.pFirst, m_keyPointsArray.ElementSize, m_keyPointsArray.Size, pfile) != m_keyPointsArray.Size)
				return 0;
		}
		if (m_primitivesArray.Size)
		{
...
void NLPATH_GEOMETRY::getPathPoint(NLPATH_POINT *pres, const Nf32 s) const
{
	...
	// Recherche de S
	NLPATH_POINT *pkp1 = ((NLPATH_POINT *)m_keyPointsArray.pFirst) + 1;
...
void NLPATH_GEOMETRY::getPathPoint(NLPATH_POINT *pres, const NLPATH_POINT *pkp1, const NLPATH_PRIMITIVE *pprim, const Nf32 s) const
{
	...
	/*
	#ifdef _DEBUG
	NLPATH_POINT *dbg_pkp1 = ((NLPATH_POINT*)m_keyPointsArray.pFirst) + 1;
	...
	*/
	...
	case NLPATH_PRIMITIVE_ID_CLOTHOID:
		if (ISFLAG_ON(pprim->m_core.m_flags, FLAG_NLPATH_CLOTHOID_SECOND))
		{
			...
			pres->p.x = pkp1->p.x + pprim->m_clothoid.m_i.x * x + pprim->m_clothoid.m_j.x * y;
			pres->p.y = pkp1->p.y + pprim->m_clothoid.m_i.y * x + pprim->m_clothoid.m_j.y * y;

			// tangente en S:
			theta = pprim->m_clothoid.m_sigma * slocal * slocal / 2.0f;
			x = cos(theta);
			y = sin(theta);
			pres->u.x = -pprim->m_clothoid.m_i.x * x - pprim->m_clothoid.m_j.x * y;
			pres->u.y = -pprim->m_clothoid.m_i.y * x - pprim->m_clothoid.m_j.y * y;
		...
		else
		{
			...
			pres->p.x = pkp0->p.x + pprim->m_clothoid.m_i.x * x + pprim->m_clothoid.m_j.x * y;
			pres->p.y = pkp0->p.y + pprim->m_clothoid.m_i.y * x + pprim->m_clothoid.m_j.y * y;
...
{
	if (index < m_keyPointsArray.Size)
		return (NLPATH_POINT *)NGetArrayIndexPtr(&m_keyPointsArray, index);
	else
		return nullptr;
}
...
void NLPATH_GEOMETRY::getPathPoint(NLPATH_POINT_S *pres, const NLPATH_POINT *pkp1, const NLPATH_PRIMITIVE *pprim, const Nf32 s) const
{
	...
	*/
	...
	case NLPATH_PRIMITIVE_ID_ARC:
		...
		pres->p.x = pprim->m_arc.m_center.x + pprim->m_arc.m_i.x * x + pprim->m_arc.m_j.x * y;
		pres->p.y = pprim->m_arc.m_center.y + pprim->m_arc.m_i.y * x + pprim->m_arc.m_j.y * y;
		pres->k = pkp1->k;
		return;
...
void NLPATH_GEOMETRY::draw(const NMATRIX *pmx, const Nf32 path_width)
{
	...
	if (m_primitivesArray.Size)
	{
		...
		// Tracage Keypoints
		pkp0 = (NLPATH_POINT *)m_keyPointsArray.pFirst;
		for (i = 0; i < m_keyPointsArray.Size; i++, pkp0++)
		{
